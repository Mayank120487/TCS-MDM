...
module namespace SearchTextInOrchestraReplication = "http://marklogic.com/rest-api/resource/SearchTextInOrchestraReplication";
import module namespace search = "http://marklogic.com/appservices/search" at "/MarkLogic/appservices/search/search.xqy";
declare namespace es = "http://marklogic.com/entity-services";

declare function SearchTextInOrchestraReplication:post($context as map:map, $params  as map:map,$input as document-node()*) as document-node()?
{
    xdmp:log("SERVICE-ENTRY: SearchTextInOrchestraReplication"),
                                     ...
                                     let $searchs := search:search($qtext,$options) (:,$StartIndex,$pageSize):)
                                      return <SearchResponse MatchedCount = "{$searchs/@total}">{  for $search at $c in $searchs//search:result
                                                                     return  <MatchResult Index="{$c}">{
                                                                                 (:<URI>{$search/@uri}</URI>,:)
                                                                                 <MatchXML>{fn:doc($search/@uri/string())/OrchestraPart/Part/PartNumber/text()}</MatchXML>(:,
	...
	xdmp:log("SERVICE-EXIT: SearchTextInOrchestraReplication")
